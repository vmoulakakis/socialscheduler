...
class SchedulerV2Tests(unittest.TestCase):
    ...
    def test_migrated_existing_buffer_post_is_reconciled_not_duplicated(self):
        ...
        scheduler.reconcile_and_fill(scheduler.expand(), posts)
        self.assertTrue(any(
            a["type"] == "already_scheduled" and a["campaign"] == "migrated" and a["postId"] == "buffer-existing"
    ...
    def test_expired_socialmarket_time_is_never_deferred_by_executor(self):
        scheduler = SocialScheduler(
            FakeClient(), SETTINGS, CHANNELS,
            backlog_item("expired", "2020-01-01T10:00:00+02:00", "Expired execution"),
            mode="dry-run",
        )
