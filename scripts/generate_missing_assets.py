#!/usr/bin/env python3
"""Generate deterministic zero-cost social posters for approved content missing media.

Source: public read-only Supabase view `socialscheduler_missing_asset_feed_v`.
Output: PNG files committed to assets/generated/<content_item_id>.png plus manifest JSON.
No paid image API is required. QR codes encode the exact tracking URL.
"""
...
def fetch_rows():
    ...
    url = f"{SUPABASE_URL}/rest/v1/socialscheduler_missing_asset_feed_v?select={urllib.parse.quote(select, safe=',')}&limit={LIMIT}"
...
def make_poster(row):
    ...
    brand_f = font(30, True)
    title_f = font(64, True)
    body_f = font(31, False)
    cta_f = font(31, True)
...
def main():
    ...
    payload = {"generated_at": datetime.now(timezone.utc).isoformat(), "source": "socialscheduler_missing_asset_feed_v", "count": len(manifest), "items": manifest}
